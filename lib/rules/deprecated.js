...
function cleanVersion (version) {
  const results = SEM_VER_REG_EXP.exec(version);
...
module.exports = {
  ...
  create (context) {
    let [ currentVersion ] = context.options;
    ...
    return {
      '*' (node) {
        ...
        const [ jsdoc ] = commentParser(`/*${lastComment}*/`);

        if (!jsdoc) {
          return;
        }

        const deprecatedTags = findTags(jsdoc, 'deprecated');
        ...
        if (!result && currentVersion !== sinceVersion || result <= 0) {
          ...
          if (reasonTag && reasonTag.description) {
            msg = `${msg} (Reason: "${reasonTag.description}")`;
          }
